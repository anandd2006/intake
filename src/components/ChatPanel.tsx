...
function classificationNotice(
  ...
  switch (classification) {
    case 'qualified':
      return {
        type: 'qualified',
        text: "Great news — this looks like a strong fit! Here's the brief I've put together.",
      }
    case 'out_of_scope':
      return {
        type: 'out_of_scope',
...
/** Fetch the public widget config (freelancer name + tagline) once. */
...
export function ChatPanel({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: `Hi there! 👋 I'm ${title}. Tell me about your project and I'll help get things started.`,
    },
  ])
  ...
  const handleNewChat = useCallback(() => {
    ...
    setRestoring(false)
  }, [sending, listening, stopListening, title])

  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-xl">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border bg-foreground px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            {tagline && <p className="truncate text-xs text-white/60">{tagline}</p>}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-background px-4 py-3">
        {restoring ? (
          ...
          <div className="space-y-3" ref={msgAnimRef}>
            ...
            {referral && (
              <AnimateIn from="fade" duration={200} delay={60}>
            ...
            {briefPanel && (
              <AnimateIn from="fade" duration={250} delay={80}>
                <div className="rounded-xl border border-border bg-white p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <FileText className="h-4 w-4 text-primary" aria-hidden="true" />
                      <p className="text-sm font-semibold text-foreground">Your Brief</p>
                    </div>
                    <button
                      onClick={() => setBriefPanel(null)}
                      className="cursor-pointer rounded-md p-1 text-foreground/40 transition-colors duration-150 hover:text-foreground"
                  ...
                  <dl className="space-y-2 text-sm">
                    {briefPanel.client_contact && (
                      <div className="flex items-start gap-2">
                        <User className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                        <dd className="text-foreground">{briefPanel.client_contact}</dd>
                      </div>
                    )}
                    <div className="flex items-start gap-2">
                      <Briefcase className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                      <dd className="text-foreground">{briefPanel.project_type}</dd>
                    </div>
                    <div className="flex items-start gap-2">
                      <FileText className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                      <dd className="text-foreground/80">{briefPanel.scope_summary}</dd>
                    </div>
                    {briefPanel.budget && (
                      <div className="flex items-start gap-2">
                        <DollarSign className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                        <dd className="text-foreground">{briefPanel.budget}</dd>
                      </div>
                    )}
                    {briefPanel.timeline && (
                      <div className="flex items-start gap-2">
                        <Calendar className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                        <dd className="text-foreground">{briefPanel.timeline}</dd>
                      </div>
                    )}
                    {briefPanel.urgency && (
                      <div className="flex items-start gap-2">
                        <Clock className="mt-0.5 h-3.5 w-3.5 shrink-0 text-foreground/40" aria-hidden="true" />
                        <dd className="text-foreground">{briefPanel.urgency}</dd>
                      </div>
                    )}
                  </dl>
                  ...
                  <div className="mt-3 flex items-center gap-1.5 text-xs text-emerald-700">
                    ...
                    Shared with {title} — they'll reach out shortly.
                  </div>
                </div>
              </AnimateIn>
            )}
